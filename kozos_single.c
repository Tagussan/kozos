...
static kz_thread_id_t thread_recv(kz_msgbox_id_t id, int *sizep, char **pp)
{
  ...
  if (mboxp->head == NULL) {
     ...
     */
    /* シングルスレッド処理では受信待ちは無く即時終了なのでNULLに戻す */
    mboxp->receiver = NULL;
    return -1;
  }
...
#else
  /*
   * システムコール処理で current が書き変わってしまうが，
   * シングルスレッド処理では継続して処理が進むので，current を戻しておく．
   * (でないと次回にシステムコールが呼ばれたときに current 不正で誤動作する)
   */
  save = current;
  syscall_intr();
  current = save;
