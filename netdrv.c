...
static void send_packet(struct netbuf *buf)
{
  ...
  if (send_buf_head) {
    if (!buf || !ether_intr_is_send_enable(0)) {
      /* 先頭から抜き出す */
      buf = send_buf_head;
	...
	send_buf_tail = &send_buf_head;
       ...
       */
      if (!ether_intr_is_send_enable(0)) {
	ether_intr_send_enable(0); /* 送信割込み有効化 */
      }

      ether_send(0, buf->size, buf->top);
      kz_kmfree(buf);
    }
  }
}
...
static void netdrv_intr(void)
{
  ...
  if (ether_is_recv_enable(0)) {
    ether_intr_clear_recv(0);
  ...
  if (ether_is_send_enable(0)) {
    ether_intr_clear_send(0);
    pkt = kx_kmalloc(DEFAULT_NETBUF_SIZE);
    memset(pkt, 0, DEFAULT_NETBUF_SIZE);
...
static int netdrv_proc(struct netbuf *buf)
{
  ...
  case NETDRV_CMD_USE: /* イーサネット・ドライバの使用開始 */
    /* ether_init(0, my_macaddr); */ /* netdrv_init()で行っているので不要 */
    ether_intr_recv_enable(0); /* 受信割込み有効化(受信開始) */
...
int netdrv_main(int argc, char *argv[])
{
  ...
  if (kz_recv(MSGBOX_ID_NETPROC, NULL, (void *)&buf) != -1) {
    ret = netdrv_proc(buf);
